...
impl DeviceHandle {
    fn new() -> Self {
        ...
        let video_device_config = DeviceConfig::new()
                .non_blocking_dqbuf();
        let video_device = Arc::new(VideoDevice::open(video_device_path, video_device_config)
    ...
    fn alloc_request(&self) -> ioctl::Request {
        ioctl::Request::alloc(&self.media_device)
            .expect("Failed to alloc request handle")
    }
...
impl V4l2Device {
    ...
    pub fn set_resolution(&mut self, resolution: Resolution) -> &mut Self {
        self.handle.borrow_mut().output_queue.set_resolution(resolution);
        self.handle.borrow_mut().capture_queue.set_resolution(resolution);
        self
    }
